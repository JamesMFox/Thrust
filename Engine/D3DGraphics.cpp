...
void D3DGraphics::DrawLineClip(Vec2 p0, Vec2 p1, D3DCOLOR color)
{
	...
	std::function<OutCode(float,float)> ComputeOutCode =
	 [xmin,xmax,ymin,ymax](float x, float y) -> OutCode
	...
	while (true) {
		if (!(outcode0 | outcode1)) { // Bitwise OR is 0. Trivially accept and get out of loop
			...
			//   x = p0.x + (1 / slope) * (ym - p0.y), where ym is ymin or ymax
			//   y = p0.y + slope * (xm - p0.x), where xm is xmin or xmax
	...
	if (accept)
	{
		DrawLine((int)(p0.x + 0.5f), (int)(p0.y + 0.5f), (int)(p1.x + 0.5f), (int)(p1.y + 0.5f), color);
	}
}
